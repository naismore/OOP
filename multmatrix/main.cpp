...
//#include <array>
//
//// Разбить на функции, чтение матрицы, вывод, перемножение.
//// std array для хранения массива
//
//using namespace std;
//
//array<array<int, 3>, 3> OpenMatrixFile(string path)
//{
//    ifstream in(path);
//    if (!in.is_open())
//    {
...
//                string value;
//                if (!(iss >> value) || value.find_first_not_of("0123456789.-") != string::npos) {
//                    cerr << "Error reading matrix element at position\n";
//                    return 1;
//                }
...
const int MATRIX_HEIGHT = 2;
const int MATRIX_WIDTH = 5;

array<int, MATRIX_WIDTH * MATRIX_HEIGHT> OpenFile(string path)
{
    ifstream in(path);
...
int main()
{
    ...
    array<int, MATRIX_HEIGHT * MATRIX_WIDTH> arr {};
    for (int i : arr)
    {
        cout << i << endl;
    }
